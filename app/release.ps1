# Build + sign a release installer (NSIS setup .exe) for the in-app updater, and generate latest.json.
#
...
#
# Full release flow:
#   1. Bump the version in these FIVE files (keep them in sync):
#        package.json
#        package-lock.json   (or just run `npm install` after bumping package.json)
...
#        -> builds + signs the installer, copies artifacts to installers\, and
#           writes installers\latest.json (UTF-8 without BOM, signature read from .sig).
...
# The built installer is named from tauri.conf.json's version, but everything
# below keys off package.json — if they disagree, we'd silently package the
# PREVIOUS release's installer still sitting in the bundle dir. Refuse to build
# out of sync.
...
if ($confVersion -ne $version -or $cargoVersion -ne $version) {
    Write-Error "Version mismatch: package.json=$version tauri.conf.json=$confVersion Cargo.toml=$cargoVersion — bump all five files (see header)."
    exit 1
}
...
# Copy artifacts to installers/ with the lowercase naming convention (product
# name lowercased, Tauri's -setup suffix kept) so URLs in latest.json stay short.
$bundleDir = "src-tauri\target\release\bundle\nsis"
$srcExe  = Join-Path $bundleDir "Claude Usage_${version}_x64-setup.exe"
$srcSig  = "$srcExe.sig"
$dstExe  = "installers\claude-usage_${version}_x64-setup.exe"
$dstSig  = "$dstExe.sig"
...
$url = "https://github.com/$repo/releases/download/v$version/claude-usage_${version}_x64-setup.exe"
$manifest = [ordered]@{
    version   = $version
    notes     = $Notes
    pub_date  = $pubDate
...
Write-Output "  gh release create v$version --title ""..."" --notes ""..."" $dstExe $dstSig installers\latest.json"
